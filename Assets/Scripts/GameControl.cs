...
public class GameControl : MonoBehaviour
{
    ...
    public bool CheckMatch(int index)
    {
        ...
        if (visibleFaces[0] == visibleFaces[1])
        {
            ...
            visibleFaces[1] = -2;
            success = true;
            canFlip = true; // Eşleşme sonrası kart çevirme izni
        }
        else
        {
            StartCoroutine(EnableCardFlipping());
        }
        return success;
    }

    IEnumerator EnableCardFlipping()
    {
        yield return new WaitForSeconds(1); // 1 saniye bekleme
        canFlip = true;
    }
