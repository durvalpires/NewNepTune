...
namespace Match3
{
	public class MatchPiece : MonoBehaviour
	{
		...
		private AudioClip doClip, reClip, miClip, faClip, solClip;
		...
		void Start()
		{
			...
			aSource = gameManager.GetComponent<AudioSource>();
			matchManager = gameManager.GetComponent<MatchManager>();
			doClip = matchManager.audioClipDo;
			reClip = matchManager.audioClipRe;
			miClip = matchManager.audioClipMi;
			faClip = matchManager.audioClipFa;
			solClip = matchManager.audioClipSol;
		}
		...
		public void Explode(float time)
		{
			...
			//renge göre nota çalma kodu:
			if(this.gameObject.GetComponent<MatchPiece>() != null)
			{
				if(mBallColorRender.sprite.name == "spund 1")
					aSource.PlayOneShot(doClip);
				//else if(mBallColorRender.sprite.name == "blue")
					//aSource.PlayOneShot(doClip);
				else if(mBallColorRender.sprite.name == "sound 2")
					aSource.PlayOneShot(reClip);
				else if(mBallColorRender.sprite.name == "sound 3")
					aSource.PlayOneShot(miClip);
				else if(mBallColorRender.sprite.name == "sound 4")
					aSource.PlayOneShot(faClip);
			}	
		}
